...
.hero{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding-bottom:70px}.hero h1{font-size:38px;line-height:1.1;margin:0 0 10px}.hero p{color:var(--muted);margin:0 0 26px}.tabs{display:flex;gap:7px;width:min(760px,100%);margin-bottom:8px}.tabs button{padding:7px 14px;font-size:12px}.tabs button.active{background:#3269d8;border-color:#5486e9}.reply{width:min(760px,100%);min-height:70px;background:#111318;border:1px solid #22262d;border-radius:20px;padding:18px;text-align:left;white-space:pre-wrap;line-height:1.55}.reply a{display:block;color:#79aeff;margin-top:8px;text-decoration:none}.reply a:hover{text-decoration:underline}.reply img{width:92px;height:64px;object-fit:cover;border-radius:9px;margin:8px 10px 0 0;vertical-align:middle}
...
<div class="top"><div><div class="brand">Assistant Jarvis</div><div class="version">V __JARVIS_VERSION__</div></div><div class="spacer"></div><button class="settings" onclick="location.href='/'" title="Settings">⚙ Settings</button></div>
<div id="status" class="status">● Windows host online</div>
<div class="hero"><h1>What can I do for you?</h1><p>One bar for Jarvis actions and private device messaging.</p><div id="tabs" class="tabs" hidden><button data-kind="web" class="active">Web</button><button data-kind="images">Images</button><button data-kind="videos">Videos</button></div><div id="reply" class="reply">Ready.</div></div>
...
<script>
let homeMode=false,lastQuery='',searchType='web';const input=document.getElementById('message'),reply=document.getElementById('reply'),target=document.getElementById('target'),modeLabel=document.getElementById('modeLabel'),tabs=document.getElementById('tabs');
...
async function runSearch(message){reply.textContent='Searching '+searchType+'…';try{const response=await fetch('/assistant',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message,search_type:searchType})});const data=await response.json();if(!response.ok)throw new Error(data.detail||'Request failed');showAssistant(data);speak(data.reply||'')}catch(e){reply.textContent='Assistant Jarvis error: '+e.message}}
