...
function showAssistant(data){reply.textContent=data.reply||'Done.';for(const source of (data.sources||[]).slice(0,3)){const link=document.createElement('a');link.href=source.url;link.target='_blank';link.rel='noopener';link.textContent=source.title||source.url;reply.appendChild(link)}}
async function send(){const message=input.value.trim();if(!message)return;input.value='';reply.textContent=homeMode?'Sending message to paired devices…':'Asking Jarvis…';try{const url=homeMode?'/messages/send':'/assistant';const response=await fetch(url,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message})});const data=await response.json();if(!response.ok)throw new Error(data.detail||'Request failed');if(homeMode){reply.textContent='You → paired devices\\n'+message}else{showAssistant(data);speak(data.reply||'');if(data.action&&data.action.type==='open_url')window.open(data.action.url,'_blank','noopener')}}catch(e){reply.textContent='Assistant Jarvis error: '+e.message}}
