...
.code{{font-size:44px;letter-spacing:9px;font-weight:800}}.small{{color:#93a1b1}}
...
.scopes{{display:grid;grid-template-columns:repeat(3,minmax(120px,1fr));gap:8px}}button{{border:0;border-radius:9px;padding:8px 12px;background:#2d6cdf;color:white;cursor:pointer;margin-top:8px}}.danger{{background:#8f3340}}
</style></head><body>
<h1>JARVIS</h1><p class='small'>Windows Host Dashboard · local administration only</p>
<div class='card'><h2>Pair a device</h2><p class='small'>This code can be used exactly once and expires after five minutes.</p><div class='code'>{escape(current_pairing_code or '—')}</div></div>
...
<div class='card'><h2>Security</h2><span class='badge'>ECDSA P-256 device authentication enabled</span><p class='small'>The dashboard listens only on 127.0.0.1. The public gateway never exposes this admin interface.</p></div>
</body></html>"""
